...
"""Este arquivo implementa o servidor Flask para gerenciar rotas e usuários."""
...
def listar_rotas():
    """
    Endpoint GET: Retorna todas as rotas cadastradas em formato JSON.
    """
...
def criar_rota():
    """
    Endpoint POST: Recebe dados de uma nova rota e insere no banco de dados.
    """
    nova_rota = request.get_json()
    conn = get_db_connection()
    cur = conn.cursor()

    # Atualizado para inserir paradas e dias_disponiveis
    sql_insert = """
		insert into rotas (origem, destino, horario, duracao, preco, motorista, ponto_embarque, ponto_desembarque, paradas, dias_disponiveis)
    ...
    cur.execute(
        ...
        (
            ...
            nova_rota.get("paradas", ""),
            nova_rota.get("dias_disponiveis", ""),
        ),
    )
...
def search_rota():
    """
    Endpoint GET: Realiza busca filtrada por origem e destino direto no banco (Opcional/Backup).
    """
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    cur.execute(
        "select * from rotas where origem ILIKE %s and destino ILIKE %s;",
        (origem, destino),
    )
    rotas = cur.fetchall()
    ...
    if rotas:
        return jsonify({"origem": origem, "destino": destino, "horario": rotas[0][3]})
    else:
        return jsonify({"message": "Nenhuma rota encontrada"}), 404


@app.route("/rotas/<int:id>", methods=["DELETE"])
def delete_rota(id):
    """
    Endpoint DELETE: Remove uma rota específica baseada no ID.
    """
    conn = get_db_connection()
    cur = conn.cursor()
...
def add_usuario():
    """
    Endpoint POST: Registra um novo usuário, aplicando hash na senha por segurança.
    """
    ...
    if not validar_senha(senha):
        return (
            jsonify({"erro": "Senha fraca. Minimo 8 caracteres com letras e numeros."}),
            400,
        )
...
def login_user():
    """
    Endpoint POST: Verifica credenciais (email/senha) e autentica o usuário.
    """
    ...
    cur.execute(
        "select nome, email, senha, telefone, tipo_usuario from usuarios where email = %s",
        (email,),
    )
...
def validar_senha(senha):
    """Verifica complexidade mínima da senha (tamanho, letras e números)."""
    if len(senha) < 8:
        return False
